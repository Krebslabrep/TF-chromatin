Load.data = function(data.type = NULL, pool.replicates = FALSE){
  ...
  if(isEmpty(grep("^sampleSheet$", current.variables))){
    if(data.type == "F1_bait.capture"){
      ...
      if(pool.replicates){sampleSheet <<- "/g/krebs/barzaghi/HTS/SMF/MM/2024-05-22-2222T2JNX/Qinput_Sox2_dTAG_run2_run3.txt"}
      else if (!pool.replicates){stop("We didn't make this one")}
...
polish.bulk.smf.plot = function(footprint.charter.results, data.type, current.sample, RegionOfInterest, plotting.TFBSs){
  ...
  plotting.colors = tryCatch({plotting.color.assignment(current.sample)}, error = function(e){NULL})
  
  footprint.charter.results$SingleSite.plot +
    geom_point(size = 2.5) +
    geom_line(linewidth = 1.25) +
    {if(!is.null(plotting.colors)){scale_color_manual(values = plotting.colors)}} +
    {if(!is.null(plotting.colors)){scale_fill_manual(values = plotting.colors)}} +
...
patch.single.site.plots = function(
  ...
  } else if (data.type %in% c("F1_amplicon", "WT_amplicon", "Rest_ko_amplicon", "Nrf1_kd_amplicon", "Nrf1_dosage_amplicon")){
  ...
  if(data.type %in% c("F1_bait.capture", "F1_amplicon", "Nrf1_kd_amplicon", "Sox2_kd_bait.capture", "Oct4_kd_bait.capture", "remodeller_bait.capture", "Nrf1_dosage_amplicon")){
